...
if fl is not None:
    filename = fl.name
    st.write(filename)
    df = pd.read_excel(filename,'DATA', usecols='B:W',header=1)
else:
    os.chdir(r"D:\hoanghung\hoc python\streamlit-app") 
    df = pd.read_excel("datafull.xlsx", 'DATA', usecols='B:W',header=1)
...
#sheet_name = 'DATA'
#df = pd.read_excel(excel_file, sheet_name=sheet_name, usecols='B:W', header=1)
...
st.subheader('Danh sách sinh viên theo cán bộ tư vấn')
st.dataframe(df[cbtv_mask])
